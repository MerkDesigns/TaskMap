...
Renderer-v2's production DOM viewport, its isolated transient camera controller, and the retained
pure geometry and viewport-culling calculations. The camera controller owns only pan/zoom state and
animation-frame presentation; the broader Phase 4 interaction controllers remain at the application
boundary under `src/app/interactions/`.

`src/canvas/interaction/canvasCameraSession.ts` owns the renderer-session-only per-canvas camera
cache. Canvas switches save and restore this cache at gesture/wheel-settle boundaries; raw pointer
samples never enter Redux, history, or persistence.
...
- `src/features/canvases/` retains document-backed canvas workspace command adapters for the later
  architecture-v1 port. It does not own a second Renderer V2 Canvas Browser presentation.
...
TaskMap theme foundation. `src/ui/materials/liquid-dom/` retains shared material roles and optics.
`src/ui/renderer-v2-prototype/` is the one canonical Renderer V2 UI: it owns the synthetic scene,
normal DOM canvas, Liquid Canvas Browser/Card runtime, and in-prototype diagnostics. Canvas elements
remain ordinary React/DOM content and do not use Liquid DOM materials.

The prototype uses one coarse backdrop `Html`, one Large Panel Canvas Browser container, one shared
Small Panel Canvas Card container, Group-based layout/scrolling, and a temporary Liquid container
for the physically dragged card.
...
- `src/canvas/RendererV2CanvasViewport.tsx` and adjacent CSS — the production ordinary-DOM canvas
  surface, transformed world layer, infinite dotted background, and pointer/wheel event adapter.
- `src/canvas/interaction/rendererV2CameraController.ts` — renderer-local, imperative pan/zoom
  controller with animation-frame presentation batching and no Redux or persistence dependency.
...
- `src/ui/materials/materialTypes.ts` and `materialDefinitions.ts` — minimal discriminated material
  contract, exact Large/Small/Opaque/Cutout surfaces, and the one shared acrylic cache profile.
...
| `src/app/AppShell.test.tsx`                                                  |    63 | Tests for the adjacent module                                                               |
| `src/app/AppShell.tsx`                                                       |    12 | React component or typed UI module                                                          |
...
| `src/app/hooks.ts`                                                           |     7 | TypeScript application module                                                               |
...
| `src/canvas/interaction/canvasCameraSession.test.ts`                         |    20 | Tests for the adjacent module                                                               |
...
| `src/domain/commands/core/canvasCommands.test.ts`                            |   126 | @vitest-environment node                                                                    |
| `src/domain/commands/core/canvasCommands.ts`                                 |   150 | TypeScript application module                                                               |
...
| `src/features/canvases/canvasWorkspaceCommands.test.ts`                      |    48 | Tests for the adjacent module                                                               |
| `src/features/canvases/canvasWorkspaceCommands.ts`                           |    73 | TypeScript application module                                                               |
...
| `src/main.tsx`                                                               |    15 | TypeScript application module                                                               |
...
| `src/ui/materials/liquid-dom/index.ts`                                       |     9 | TypeScript application module                                                               |
...
| `src/ui/materials/liquid-dom/liquidDomRuntime.test.ts`                       |   197 | Tests for the adjacent module                                                               |
| `src/ui/materials/liquid-dom/liquidDomRuntime.ts`                            |   152 | The Html host owns interaction when a surface contains normal DOM controls.                 |
| `src/ui/materials/liquid-dom/LiquidMaterialSurface.tsx`                      |   108 | React component or typed UI module                                                          |
...
| `src/ui/materials/materialDefinitions.ts`                                    |    89 | TypeScript application module                                                               |
...
| `src/ui/renderer-v2-prototype/benchmarkCanvasCardInteraction.ts`             |   111 | TypeScript application module                                                               |
...
| `src/ui/renderer-v2-prototype/benchmarkGifAsset.ts`                          |     4 | Five-frame, development-only GIF used to force recurring native image paints.               |
...
| `src/ui/renderer-v2-prototype/canvasCardDragSession.ts`                      |    71 | TypeScript application module                                                               |
| `src/ui/renderer-v2-prototype/canvasCardPointerSession.ts`                   |    53 | TypeScript application module                                                               |
| `src/ui/renderer-v2-prototype/canvasCardRuntimeTestSupport.ts`               |    27 | TypeScript application module                                                               |
| `src/ui/renderer-v2-prototype/dev/BenchmarkControls.test.tsx`                |    57 | Tests for the adjacent module                                                               |
| `src/ui/renderer-v2-prototype/dev/BenchmarkControls.tsx`                     |   215 | DEV/PROTOTYPE ONLY — do not port with Renderer V2 production implementation.                |
| `src/ui/renderer-v2-prototype/dev/BenchmarkMaterialControls.tsx`             |    90 | DEV/PROTOTYPE ONLY — do not port with Renderer V2 production implementation.                |
| `src/ui/renderer-v2-prototype/dev/BenchmarkMaterialMenu.tsx`                 |   306 | DEV/PROTOTYPE ONLY — do not port with Renderer V2 production implementation.                |
| `src/ui/renderer-v2-prototype/dev/benchmarkMetrics.test.ts`                  |    53 | @vitest-environment node                                                                    |
| `src/ui/renderer-v2-prototype/dev/benchmarkMetrics.ts`                       |   159 | DEV/PROTOTYPE ONLY — do not port with Renderer V2 production implementation.                |
...
| `src/ui/renderer-v2-prototype/dev/canvasBrowserRuntimeMetrics.test.ts`       |    28 | Tests for the adjacent module                                                               |
| `src/ui/renderer-v2-prototype/dev/canvasBrowserRuntimeMetrics.ts`            |    77 | DEV/PROTOTYPE ONLY — benchmark aggregation for optional Canvas Browser instrumentation.     |
...
| `src/ui/renderer-v2-prototype/dev/liquidCaptureProbe.ts`                     |    78 | DEV/PROTOTYPE ONLY — do not port with Renderer V2 production implementation.                |
...
| `src/ui/renderer-v2-prototype/liquidCanvasBrowserAppearance.ts`              |   119 | TypeScript application module                                                               |
| `src/ui/renderer-v2-prototype/liquidCanvasBrowserInstrumentation.ts`         |    10 | TypeScript application module                                                               |
| `src/ui/renderer-v2-prototype/liquidCanvasBrowserPresentation.ts`            |   148 | TypeScript application module                                                               |
| `src/ui/renderer-v2-prototype/liquidCanvasBrowserRuntime.ts`                 |   378 | TypeScript application module                                                               |
...
| `src/ui/renderer-v2-prototype/liquidCanvasBrowserTypes.test.ts`              |    34 | Tests for the adjacent module                                                               |
| `src/ui/renderer-v2-prototype/liquidCanvasBrowserTypes.ts`                   |    16 | TypeScript application module                                                               |
| `src/ui/renderer-v2-prototype/liquidCanvasCardFactory.ts`                    |   127 | TypeScript application module                                                               |
| `src/ui/renderer-v2-prototype/liquidCanvasCardGeometry.ts`                   |   150 | Keep the full-size Html registered in Liquid's shared content atlas. Zero-sized Glass       |
...
| `src/ui/renderer-v2-prototype/liquidSceneBenchmarkRuntime.ts`                |   265 | DEV/PROTOTYPE ONLY — capture and frame instrumentation.                                     |
...
| `src/ui/renderer-v2-prototype/useBenchmarkVisibleElements.ts`                |    65 | The store intentionally publishes a version for in-place structural additions.              |
