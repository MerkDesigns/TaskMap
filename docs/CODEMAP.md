...
- `src/features/workspace-chrome/` composes the production coarse Liquid DOM application root, the
  stable Small Panel top bar, and the Large Panel left-browser shell without owning document
  mutations. Development builds bootstrap one ephemeral document in the real Redux workspace until
  the production database-opening flow owns workspace loading; stable builds do not bootstrap.
- `src/features/canvases/` owns the real document-backed Renderer V2 Canvas Browser, lightweight
  geometry previews, named workspace-command adapters, Small Panel Canvas Cards, and the transient
  pointer/rAF/FLIP reorder presentation.
- `src/features/extensions/` owns the Renderer V2 Extension Browser search, target filtering,
  five-item local favorites, and drag-preview boundary. It filters the still-canonical legacy
  metadata registry to retained extensions until target extension modules populate
  `architectureRegistry.ts`; successful canvas drops remain deferred to element rendering.
...
development-only control verification view. `src/ui/materials/liquid-dom/` is the shared Large
Panel/Small Panel runtime and feature-facing material boundary. `src/ui/dev/renderer-benchmark/`
contains the explicitly gated A/B/C canvas-composition benchmark, its neutral ephemeral scene
model, measurement utilities, and the only approved benchmark-specific direct Liquid DOM adapter.
It is not a production canvas or document owner. Canvas elements remain ordinary React/DOM content
and do not use Liquid DOM materials.

The Liquid DOM boundary supports explicit scene order plus event-driven/imperative surface geometry
refresh for nested application chrome. Every independent surface continues to own one Container;
the production workspace uses one coarse backdrop `Html`, not one `Html` per canvas element.
...
| `scripts/check-architecture.mjs`                                          |   272 | Repository maintenance script                                                               |
| `scripts/check-phase2-production-exclusion.mjs`                           |    85 | Repository maintenance script                                                               |
...
| `src/app/AppShell.tsx`                                                    |    56 | React component or typed UI module                                                          |
...
| `src/features/canvases/useCanvasPointerReorder.test.tsx`                  |   105 | Tests for the adjacent module                                                               |
| `src/features/canvases/useCanvasPointerReorder.ts`                        |   230 | The document fallback can finish after the source is detached.                              |
| `src/features/canvases/useCanvasReorderFlip.ts`                           |    53 | TypeScript application module                                                               |
| `src/features/extensions/ExtensionBrowser.tsx`                            |   195 | React component or typed UI module                                                          |
| `src/features/extensions/extensionBrowserModel.test.ts`                   |    26 | Tests for the adjacent module                                                               |
| `src/features/extensions/extensionBrowserModel.ts`                        |    55 | TypeScript application module                                                               |
| `src/features/extensions/useExtensionDragPreview.ts`                      |    54 | TypeScript application module                                                               |
...
| `src/main.tsx`                                                            |    53 | TypeScript application module                                                               |
...
| `src/ui/dev/LiquidDomVerificationFixture.test.tsx`                        |    68 | Tests for the adjacent module                                                               |
| `src/ui/dev/LiquidDomVerificationFixture.tsx`                             |   234 | React component or typed UI module                                                          |
